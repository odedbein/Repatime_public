...
#for this participant, he moved before the color test, so color tests are aligned based on a FM that I collected after the color test.
#I did the prep manually, then just corrected based on day2_FieldMap_PA3/AP3
...
#run topup - day2 - third
echo "estimating b0 bias with topup - day2_pair3"
fm_pair=$sub_dir/$fm_dir/day2_fm_pair3.nii.gz

topup --imain=$fm_pair \
      --datain=$fm_params \
      --config=b02b0.cnf \
      --out=$sub_dir/$fm_dir/day2_topup3 \
      --iout=$sub_dir/$fm_dir/day2_fm_pair_unwarped3
#      --verbose
