function compile_rsa_raw_data(ResultsEncSimOnlyNum,engram,file_init,model)
...
Rfiles_dir=fullfile(results_dir,'FilesForR_outlier_corr_removed/regions_data');
...
tr=1;reg=reg_names{1};items=allOrResp{1};
%take one region just to have the size:
curr_data=ResultsEncSimOnlyNum.(TRs{tr}).(reg).byList.SimMat.(items);
...
for r=regions
    ...
    for tr=1:numel(TRs)
        ...
        curr_data=ResultsEncSimOnlyNum.(TRs{tr}).(reg).byList.SimMat.all_items; %I took curr_data again, participants are first dim.
        %fisher transform the data, that is usually desireable.
        if Fisher
            curr_data=atanh(curr_data);
        end
        %calculate the diff from rep1:
        first_rep=repmat(curr_data(:,:,:,1,:),[1,1,1,5,1]);
