function compile_rsa_raw_data_noRep1Subtraction(ResultsEncSimOnlyNum,engram,file_init,model)
...
%fisher transform the data, that is usually desireable.
Fisher=1;
...
curr_data=ResultsEncSimOnlyNum.(TRs{tr}).(reg).byList.SimMat.all_items;
%curr_data=ResultsEncSimOnlyNum; %participants are the first dim, make them the last dim, just for ease
...
for r=regions
    ...
    for tr=1:numel(TRs)
        ...
        curr_data=ResultsEncSimOnlyNum.(TRs{tr}).(reg).byList.SimMat.all_items;
        %curr_data=ResultsEncSimOnlyNum; 
        %fisher transform the data, that is usually desireable.
        if Fisher
        ...
        if remove_outliers
            for subj=1:nSubj
                ...
                curr_data_temp=squeeze(data_diff1stRep(subj,:,:,:,:));
    ...
    T.Properties.VariableNames=TRs;
    ...
    results_fname=[file_init '_' reg '_simMat' results_file_ext];
